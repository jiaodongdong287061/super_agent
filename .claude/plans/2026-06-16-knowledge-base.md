...
class SemanticChunker(BaseChunker):
    ...
    def _split_large_section(
        ...
        chunk_first_sent_idx = 0  # 当前 chunk 的首句在 sent_positions 中的索引
...
class Indexer:
    ...
    def _build_single_file(self, file_path: Path, **kwargs) -> None:
        ...
        for doc in documents:
            doc.metadata["doc_version"] = str(int(old_state.get("version", "0")) + 1) \
                if isinstance(old_state, dict) and old_state.get("hash") != file_hash \
                else old_state.get("version", "0")
    ...
    def _build_directory(self, doc_path: Path, file_tags: dict[str, list[str]] | None = None, **kwargs) -> None:
        ...
        for fp in doc_path.rglob("*"):
            ...
            state[rel_path] = {"hash": file_hash, "version": new_version, "last_indexed": ..., "chunk_ids": [...]}
            self._save_state(state)
    ...
    def _index_chunks(self, chunks: list, rel_path: str) -> None:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {executor.submit(_process_batch, i, b): i for i, b in enumerate(batches)}
...
| F1 语义结构切分 + 跨页页码归属 | 任务 6 |
| F1 5页合并分组 + char_start 追踪 | 任务 4, 任务 6 |
...
| 页码存储 page_numbers + char_start 偏移追踪 | 任务 3, 任务 6 |
