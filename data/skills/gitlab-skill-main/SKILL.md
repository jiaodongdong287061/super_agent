---
name: gitlab-skill
description: GitLab integration for fetching issues/MRs, generating executive summaries, performing comprehensive code reviews, automating issue resolution workflows, and managing projects (create, fork, branch). Use when working with GitLab issues, merge requests, projects, or when asked to summarize, review, comment on, resolve, create, or fork GitLab work items.
---
...
This skill provides comprehensive GitLab integration capabilities, enabling automated workflows for issue management, code review, project management, and project insights. It bridges GitLab data with intelligent analysis and action, supporting both read operations (summaries, reviews) and write operations (posting comments, creating merge requests, creating projects, forking projects).

## When to Use This Skill

Invoke this skill when:
- Creating new GitLab projects with default branches (dev, test, prod)
- Forking existing GitLab projects
- Creating branches in a project
...
### Workflow 0: Create a New Project

**Trigger Examples:**
- "Create a new project named 'my-app'"
- "Create a GitLab project called 'backend-api'"
- "Initialize a new project 'webapp' in namespace 123"

**Steps:**

1. **Create the project:**
   ```bash
   python scripts/gitlab_api.py create-project <name> [namespace_id] [visibility] [description]
   ```
   This automatically:
   - Creates a new GitLab project
   - Initializes it with a README
   - Creates dev, test, and prod branches from main

2. **Parameters:**
   - `name` (required): Project name
   - `namespace_id` (optional): Namespace/group ID
...
3. **Example:**
   ...
   python scripts/gitlab_api.py create-project "my-new-project"

   # Create a public project with description
   python scripts/gitlab_api.py create-project "my-app" "" "public" "My application"

   # Create project in a specific namespace
   python scripts/gitlab_api.py create-project "team-api" 45 "private" "Team API project"
   ```

4. **Output:**
   Returns project JSON including:
   - Project ID, name, path
   - HTTP/SSH URLs
   - `created_branches`: ["dev", "test", "prod"]

### Workflow 0.1: Fork a Project

**Trigger Examples:**
- "Fork project acme/webapp"
- "Fork the webapp project to my namespace"
- "Create a fork of project 123"

**Steps:**

1. **Fork the project:**
   ```bash
   python scripts/gitlab_api.py fork-project <project> [namespace] [name] [path]
   ```

2. **Parameters:**
   - `project` (required): Source project ID or alias
   - `namespace` (optional): Target namespace/username
   - `name` (optional): New project name
   - `path` (optional): New project path

3. **Example:**
   ```bash
   # Fork to current user's namespace
   python scripts/gitlab_api.py fork-project "acme/webapp"

   # Fork to a specific namespace with new name
   python scripts/gitlab_api.py fork-project "webapp" "my-team" "my-webapp" "my-webapp"
   ```
...
- `create-project <name> [namespace_id] [visibility] [description]` - Create new project with dev/test/prod branches
- `fork-project <project> [namespace] [name] [path]` - Fork a project
- `create-branch <project> <branch> <ref>` - Create a branch in a project
- `delete-project <project>` - Delete a project (WARNING: permanent)
...
# Create a new project with dev/test/prod branches
python scripts/gitlab_api.py create-project "my-new-project"
python scripts/gitlab_api.py create-project "team-api" 45 "private" "Team API service"
...
# Create a branch
python scripts/gitlab_api.py create-branch "acme/webapp" "feature/login" "main"

# Delete a project (WARNING: permanent)
python scripts/gitlab_api.py delete-project "acme/old-project"
```
